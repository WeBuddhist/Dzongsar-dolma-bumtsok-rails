...
> `^toc-X-Y-Z` indexes lines in the outline block it writes (`4-SYSTEM/Guidelines/annotation-conventions.md`
> §4).
>
> **They are never used for a source file's headings.** As a heading anchor in
> `1-SOURCES/`, `^TOC-N` is deprecated and wrong (`4-SYSTEM/Guidelines/annotation-conventions.md` §6): the
...
- [ ] `^TOC-` IDs appear only in these two Adaptation files — no `^TOC-` anchor written onto any heading in `1-SOURCES/`
