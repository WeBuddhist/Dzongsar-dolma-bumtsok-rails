...
description: >
  ...
  markdown heading lines with block IDs per 4-SYSTEM/Guidelines/annotation-conventions.md section 5.
  Run after a format-*-root-text skill, on a working copy in 0-INBOX/temp/ — the
  result replaces the canonical file under 1-SOURCES/ only after human review.
...
This implements the convention in `4-SYSTEM/Guidelines/annotation-conventions.md` §5 ("Inline TOC
wikilinks"). The output is saved to `0-INBOX/temp/` and is **not** yet the canonical
file under `1-SOURCES/`; human review is required before it replaces that file.
...
```
  the canonical file under 1-SOURCES/  (read-only input)
       │
   ...
   5. Render:     assign block IDs from depth, insert headings, wrap wikilinks by anchored
       ...
       ▼
  0-INBOX/temp/tagged-<filename>.md
```
...
| Field | Description |
|---|---|
| Input file | Path to a formatted text file — normally the canonical file under `1-SOURCES/`, or a working copy of it under `0-INBOX/temp/` |
...
A single file at `0-INBOX/temp/tagged-<original-filename>` (the
...
`0-INBOX/temp/<filename>.annotation.json`.
...
python3 4-SYSTEM/Skills/tag-inline-toc/scripts/tag_inline_toc.py render \
    --input  "1-SOURCES/<filename>.md" \
    --annot  "0-INBOX/temp/<filename>.annotation.json" \
    --output "0-INBOX/temp/tagged-<filename>.md"
```
...
python3 4-SYSTEM/Skills/tag-inline-toc/scripts/tag_inline_toc.py verify \
    --input <input-file> --tagged "0-INBOX/temp/tagged-<filename>.md"
```

Then report to the user: the render counts, the output path, and every node
you marked `"review"`. The output stays in `0-INBOX/temp/` — it is **not** the
...
8. **Output goes to `0-INBOX/temp/`**, never overwrites the canonical file under `1-SOURCES/` directly. Promoting the tagged copy over that file is a human decision, taken after review.
...
- [ ] Output written to `0-INBOX/temp/tagged-<filename>.md` — the canonical file under `1-SOURCES/` untouched; review nodes reported to the user
...
and `0-INBOX/temp/`) are now the logical name `0-INBOX/temp/`, resolved per repo from
`4-SYSTEM/Guidelines/skill-locations.md`; `tag_inline_toc.py`'s default output-path helper
...
`4-SYSTEM/Guidelines/annotation-conventions.md` §5. Marked OPTIONAL — most texts have no inline
...
document it cited there is now `4-SYSTEM/Guidelines/annotation-conventions.md` §5.
